...
from app.schemas.common import WritingIn, WritingOut, WritingListOut
...
def unique_slug(title,db,exclude=None):
 ...
 return slug
@router.get("", response_model=list[WritingListOut])
def list_public(
    ...
    s = (
        select(Writing)
        .options(defer(Writing.content))
        .where(Writing.status == "PUBLISHED")
    )

    if q:
        s = s.where(
            or_(
                Writing.title.ilike(f"%{q}%"),
                Writing.content.ilike(f"%{q}%"),
                Writing.excerpt.ilike(f"%{q}%")
            )
        )

    if category_id is not None:
        s = s.where(Writing.category_id == category_id)

    if featured is not None:
        s = s.where(Writing.featured == featured)

    return list(
        db.scalars(
            s.order_by(
                Writing.published_at.desc().nullslast()
            )
            .offset((page - 1) * limit)
            .limit(limit)
        ).all()
